  ...
  <div class="page">
    ...
    <div class="my-content">
      ...
      <el-tag type="info"
              class="info-box"
              v-show="active === 0">
        <h1>Hi {{ username }},对商品有什么需要修改的地方，在这里修改吧～</h1>
      </el-tag>
      <h1>{{ titleMsg }}</h1>
...
export default {
  ...
  mounted () {
    ...
    that.$axios.get('/goods_api/goods', {
      ...
      this.form.type = res.data[0].type_key
      this.form.price = res.data[0].price
      this.form.brand = res.data[0].brand
      this.form.condition = res.data[0].condition
      this.form.detail = res.data[0].detail
      this.form.want = res.data[0].want
      for (let item of res.data[0].picture) {
        this.form.picture += (item + '$$$')
        this.fileList.push({ url: this.baseUrl + item })
      }
    })
  },
  methods: {
    submitForm () {
      ...
      this.$confirm('确定信息无误提交吗，亲？', '发布确认', {
        ...
        that.$axios.put('/goods/goods', this.form).then(res => {
          // 发布成功
          if (res.data.flag === 'success') {
            this.$notify.success({ title: '发布成功',
              message: '发布修改成功' })
            this.$router.push('/personal/publish')
          } else {
    ...
    handleRemove (file, fileList) {
      ...
      if (file.response === undefined) removeBaseUrl = file.url.split('/upload/')[1]
      else removeBaseUrl = file.response.url
      let removeUrl = removeBaseUrl + '$$$'
